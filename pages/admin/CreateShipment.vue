<template>
  <main class="space-y-6 py-20">
    ...
    <!-- Notification Message for 'Add Box' -->
    <div v-if="notificationMessage" class="bg-yellow-200 p-4 rounded text-center text-black">
      {{ notificationMessage }}
    </div>

    <!-- Success Message -->
    <div v-if="successMessage" class="bg-green-200 p-4 rounded text-center text-black">
      {{ successMessage }}
    </div>
      ...
      <ul class="space-y-4">
        <li v-for="field in box.fields" :key="field.id" class="flex items-center gap-1">
          ...
          <span @click="removeFields(box.id, field.id)">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6 text-red-600">
              <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 00 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"/>
            </svg>
          </span>
        </li>
      </ul>
...
// Handle the "Create Shipment" button click
const handleCreateShipment = async () => {
  if (boxes.value.length === 0) {
    // Show the notification if no boxes are added
    notificationMessage.value = "Please add at least one box before creating a shipment.";
...
const saveToDatabase = async () => {
  try {
    ...
    // Reset the state
    boxes.value = []; // Clear the boxes
    notificationMessage.value = ""; // Clear any notifications
    showSuccessMessage.value = false; // Close the success message if any

<template>
      ...
      <div v-if="foundShipment" class="max-w-3xl mx-auto p-6 bg-white  rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
          <div class=" p-3 rounded-sm ">
            ...
            <p class="text-sm md:text-xl text-gray-700"><strong>Name:</strong> {{ shipmentData.shipper.name }}</p>
            <p class="text-sm md:text-xl text-gray-700"><strong>Address:</strong> {{ shipmentData.shipper.address }}</p>
            <p class="text-sm md:text-xl text-gray-700"><strong>Phone:</strong> {{ shipmentData.shipper.phoneNumber }}</p>
            <p class="text-sm md:text-xl text-gray-700"><strong>Email:</strong> {{ shipmentData.shipper.email }}</p>
          </div>
          <hr class="sma:hidden ml-3 w-11/12">
          <div class=" p-3 rounded-sm ">
            <h3 class="text-sm md:text-xl font-semibold text-gray-500 mb-3 ">Receiver Info</h3>
            <p class="text-sm md:text-xl text-gray-700"><strong>Name:</strong> {{ shipmentData.receiver.name }}</p>
            <p class="text-sm md:text-xl text-gray-700"><strong>Address:</strong> {{ shipmentData.receiver.address }}</p>
            <p class="text-sm md:text-xl text-gray-700"><strong>Phone:</strong> {{ shipmentData.receiver.phoneNumber }}</p>
            <p class="text-sm md:text-xl text-gray-700"><strong>Email:</strong> {{ shipmentData.receiver.email }}</p>
          </div>
        </div>
        <hr class="sma:hidden ml-3 w-11/12">
        ...
        <div v-if="shipmentData.shipmentImages?.length" class="mb-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div v-for="(image, index) in shipmentData.shipmentImages" :key="index" class="relative group">
              <img :src="image" alt="Shipment Image" class="size-48 object-cover rounded-lg -md">
              <a :href="image" target="_blank" class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 flex items-center justify-center text-white text-sm font-semibold transition">
                View Full Image
              </a>
            </div>
          </div>
        </div>
        <p v-else class="text-gray-500 text-sm">No images available for this shipment.</p>
  
        <div class="mb-4">
          <h3 class="text-sm md:text-xl font-semibold text-gray-500 mb-3 ">Shipment Details</h3>
          <ul v-if="shipmentData.details?.length" class="list-inside list-disc text-gray-700">
            <li v-for="(detail, index) in shipmentData.details" :key="index" class="text-sm">
        ...
        <div class="text-center mt-6">
          <client-only>
    <button @click="printReceipt" class="px-4 py-2 rounded-md bg-blue-600 text-white">
      Print Receipt
    </button>
  </client-only>
        ...
        <div class="text-center text-gray-600 mt-12">
          <p>Shipment information is provided for reference purposes only. Please contact customer support for further inquiries.</p>
        </div>
      </div>
    </div>
  </template>
  ...
  const trackShipment = async () => {
    ...
    try {
      ...
      const q = query(shipmentsRef, where("trackingID", "==", trackingInput.value.trim()));
